...
export default function ReceivereMgr(props) {
  ...
  const [selectedOption, setSelectedOption] = useState("อนุมัติให้แก้ไข"); // default value
  ...
  const handleSubmit = async (e) => {
    ...
    data.sign_create_mgr = signatureData;
    // data.file = file;
    // data.complaint_id = complaint_id;
    // data.sign_create_receiver = signatureData;
    data.receiver_mgr = user;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await API.put(ApiRouter.receiverMgr + complaint_id, data);
          ...
          <div className="rounded-t bg-white mb-0 px-6 py-6">
            <div className="text-center flex justify-between">
              <h6 className="text-gray-700 text-xl font-bold">
                ผู้จัดการต้นสังกัดที่ถูกร้องเรียน อนุมัติแก้ไข
              </h6>
            </div>
          </div>
          {status_no === "2" ||status_no === "3" || status_no === "4" || status_no === "5" ? (
                  ...
                  <div className="w-full lg:w-6/12 px-4">
                    <div className="relative w-full mb-3">
                      ...
                      <input
                        ...
                        name="sign_create_mgr"
                        defaultValue={
                          oldData.length === 0 ? user : oldData[0].receiver_mgr
                        }
                        disabled={true}
                      />
                    </div>
                  ...
                  <div className="w-full lg:w-6/12 px-4">
                    <div className="relative w-full mb-3">
                      ...
                      {oldData.length === 0 ? (
                        ...
                        >
                          <option value="อนุมัติให้แก้ไข">
                            อนุมัติให้แก้ไข
                          </option>
                          <option value="ไม่อนุมัติให้แก้ไข">
                            ไม่อนุมัติให้แก้ไข
                          </option>
                        </select>
                      ) : (
                        <input
                          type="text"
                          className="border-0 px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                          defaultValue={oldData[0].statusMgr}
                          disabled={true}
                        />
                      )}
                    </div>
                      ...
                      {oldData.length === 0 ? (
                        ...
                        <>
                          ...
                          <p className="block uppercase text-gray-600  mt-2">
                            วันที่ : {oldData[0].sign_mgr_date}
                          </p>
                        </>
                      )}
                    </div>
